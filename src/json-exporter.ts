...
export class JsonExporter {
  ...
  async export(): Promise<SchemaJson> {
  ...
  async exportToFile(outputDir: string): Promise<string> {
    ...
    if (fs.existsSync(filepath)) {
      ...
      const stripTimestamp = (s: string) => s.replace(/"exportedAt":\s*"[^"]*"/, '"exportedAt": ""');
  ...
  private async extractSchemas(): Promise<string[]> {
  ...
  private async extractSequences(): Promise<SequenceJson[]> {
    const { rows } = await this.client.query(`
      SELECT
  ...
  private async extractTables(): Promise<TableJson[]> {
    ...
    for (const tbl of tables) {
      if (!this.shouldIncludeTable(tbl.schemaname, tbl.tablename)) continue;
      result.push(await this.buildTableJson(tbl.schemaname, tbl.tablename));
    }
    return result;
  }

  private async buildTableJson(schema: string, table: string): Promise<TableJson> {
    // Columns
  ...
  private async extractViews(): Promise<ViewJson[]> {
    const { rows } = await this.client.query(`
  ...
  private async extractMaterializedViews(): Promise<ViewJson[]> {
    const { rows } = await this.client.query(`
  ...
  private async extractFunctions(): Promise<FunctionJson[]> {
    const { rows } = await this.client.query(`
      SELECT
        n.nspname AS schema_name,
  ...
  private async extractTriggers(): Promise<TriggerJson[]> {
    const { rows } = await this.client.query(`
      SELECT
  ...
  private async extractIndexes(): Promise<IndexJson[]> {
    const { rows } = await this.client.query(`
      SELECT
