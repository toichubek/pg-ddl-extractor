...
exports.PreMigrationChecker = void 0;
exports.printPreCheckReport = printPreCheckReport;
...
class PreMigrationChecker {
    ...
    async checkTableBloat() {
      ...
      `);
            ...
            const bloated = rows.filter((r) => parseFloat(r.dead_pct) > 20);
            if (bloated.length > 0) {
                const detail = bloated
                    .map((r) => `${r.table_name}: ${r.n_dead_tup} dead tuples (${r.dead_pct}%)`)
                    .join(", ");
                return {
                    name: "table-bloat",
                    status: "warn",
...
exports.PreMigrationChecker = PreMigrationChecker;
// ─── Report ───────────────────────────────────────────────────
function printPreCheckReport(result) {
    console.log("═══════════════════════════════════════════════════════════");
