...
async function main() {
    ...
    try {
        ...
        if (options.preCheck) {
            try {
                const conn = await (0, cli_utils_1.connectToDatabase)(options);
                try {
                    const checker = new pre_check_1.PreMigrationChecker(conn.client);
        ...
        if (options.track && migration.commands.length > 0) {
            try {
                const conn = await (0, cli_utils_1.connectToDatabase)(options);
                try {
                    const tracker = new migration_tracker_1.MigrationTracker(conn.client);
